...
interface CanvasProps { document: MindMapDocument; selectedId?: string; editNodeId?: string; onSelect: (id?: string) => void; onMove: (id: string, position: Point) => void; onRename: (id: string, label: string) => void; onEditFinished: (id: string) => void; onConnectNodes?: (sourceId: string, targetId: string) => void }
let capturedOnConnectNodes: ((sourceId: string, targetId: string) => void) | undefined
vi.mock('../src/components/MindMapCanvas', () => ({ default: ({ document, selectedId, editNodeId, onSelect, onMove, onRename, onEditFinished, onConnectNodes }: CanvasProps) => { capturedOnConnectNodes = onConnectNodes; return <section aria-label="Mock canvas"><div data-testid="node-count">{document.nodes.length}</div><div data-testid="root-label">{document.nodes[0]?.label}</div><div data-testid="edge-count">{document.edges.length}</div><div data-testid="selected-id">{selectedId}</div><div data-testid="edit-node-id">{editNodeId}</div><button onClick={() => onSelect(document.rootIds[0])}>Select root</button><button onClick={() => editNodeId && onEditFinished(editNodeId)}>Finish edit</button><button onClick={() => onMove(document.rootIds[0], { x: 42, y: 24 })}>Move root</button><button onClick={() => onRename(document.rootIds[0], 'Renamed root')}>Rename root</button></section> } }))
...
it('handles keyboard creation, destructive confirmation, and oversized import feedback', () => { render(<App />); createRootAndSelect(); fireEvent.keyDown(window, { key: 'Tab' }); expect(screen.getByTestId('node-count')).toHaveTextContent('2'); fireEvent.click(screen.getByRole('button', { name: 'Select root' })); fireEvent.keyDown(window, { key: 'Delete' }); expect(screen.getByRole('dialog', { name: 'Delete this branch?' })).toBeInTheDocument(); fireEvent.click(screen.getByRole('button', { name: 'Cancel' })); const input = document.querySelector('input[type="file"]'); expect(input).not.toBeNull(); fireEvent.change(input!, { target: { files: [new File([new Uint8Array(1_000_001)], 'large.json', { type: 'application/json' })] } }); expect(screen.getByText('Import rejected: files must be 1 MB or smaller.')).toBeInTheDocument() })

it('selects and requests inline editing for nodes created with Tab and Enter', () => {
  render(<App />)
  createRootAndSelect()
  const rootId = screen.getByTestId('selected-id').textContent
  fireEvent.keyDown(window, { key: 'Tab' })
  const childId = screen.getByTestId('selected-id').textContent
  expect(childId).toBeTruthy()
  expect(childId).not.toBe(rootId)
  expect(screen.getByTestId('edit-node-id')).toHaveTextContent(childId!)
  fireEvent.click(screen.getByRole('button', { name: 'Finish edit' }))
  expect(screen.getByTestId('edit-node-id')).toBeEmptyDOMElement()

  fireEvent.keyDown(window, { key: 'Enter' })
  const siblingId = screen.getByTestId('selected-id').textContent
  expect(screen.getByTestId('node-count')).toHaveTextContent('3')
  expect(siblingId).toBeTruthy()
  expect(siblingId).not.toBe(childId)
  expect(screen.getByTestId('edit-node-id')).toHaveTextContent(siblingId!)
})
